...
describe('TextProcessingService', () => {
  ...
  describe('lightNormalization', () => {
    ...
    it('should remove stop words from the dictionary first', () => {
      const input =
        'Hi Please add NASA to the map, kindly and thank you sorry hello';
      const { normalizedText } = service.lightNormalization(input);
    ...
    it('should correct transposed storage keywords like vualt to vault', () => {
      const input =
        'add a vualt named ABC containing lockers: three clothes 7 shirts each and two watches 5 rolex each';
      const { normalizedText } = service.lightNormalization(input);
    ...
    it('should not rewrite toys to toss during spell-check', () => {
      const input =
        'Create storage Play Room with box Toys containing toy cars';
      const { normalizedText } = service.lightNormalization(input);
    ...
    it('should correct structural words outside the old small keyword list', () => {
      const input =
        'Create storage Garage with descrption Main Warehouse and box calld Tools contaning Hammer';
      const { normalizedText } = service.lightNormalization(input);
    ...
    it('should preserve uppercase acronyms inside otherwise normalized input', () => {
      ...
      const { normalizedText } = service.lightNormalization(input);
      expect(normalizedText).toBe(
        'add storage SZABIST and boxes for BSCS papers',
      );
    });
    ...
    it('should return empty objects for null/undefined', () => {
      expect(service.lightNormalization(null as any)).toEqual({
        normalizedText: '',
        llmBackup: '',
        typoCount: 0,
      });
      expect(service.lightNormalization(undefined as any)).toEqual({
        normalizedText: '',
        llmBackup: '',
        typoCount: 0,
      });
  ...
  describe('heavyNormalization', () => {
    ...
    it('should map synonyms to canonical names', () => {
      const parsed = {
        storageName: 'garage',
        boxes: [{ name: 'car stuff', quantity: null, description: null }],
        items: [],
      };
