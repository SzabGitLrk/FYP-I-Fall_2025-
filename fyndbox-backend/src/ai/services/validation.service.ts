...
export class ValidationService {
  ...
  intentClassification(
    ...
    if (existingContext) {
      const allDbNames = [
        ...existingContext.storages.map((s: any) =>
          typeof s === 'string' ? s : s.name,
        ),
        ...existingContext.boxes.map((b: any) =>
          typeof b === 'string' ? b : b.name,
        ),
        ...existingContext.items.map((it: any) =>
      ...
      for (const name of entityNamesToCheck) {
        ...
        if (fuzzyMatches.length > 0) {
          const candidates = [name, ...fuzzyMatches]
            .map((n) => `'${n}'`)
            .join(' or ');
    ...
    if (parsedData.meta?.preIntentLocationOverflow) {
      return {
        ...
        scope,
        clarification:
          "Please use one storage and up to two box levels. Example: 'In Kitchen in Fridge add dozen eggs'.",
        suggestions,
        confidence: 0,
    ...
    if (!intent && rawIntents.length === 0 && hasEntities) {
      const canDirectClarify =
        !hasUnrecognizedWords && !hasConversationalNoise && hasStructuredAnchor;
      return {
        intent: null,
        isValid: false,
        scope,
        ...
        suggestions,
        confidence: 0,
        shouldFallToLLM: !canDirectClarify,
      };
    }
      ...
      if (targetedItems.length > 0) {
        if (!existingContext) {
          ...
          return {
            ...
            scope,
            clarification:
              'Please confirm which existing item quantity should be decreased.',
            suggestions,
            confidence: 0,
            shouldFallToLLM: true,
          };
        ...
        const matchedStorageId =
          matchedStorageRecord && typeof matchedStorageRecord === 'object'
        ...
        const scopedBoxes = matchedStorageId
          ? existingContext.boxes.filter(
              (box: any) =>
                typeof box === 'object' &&
                (box as any).storageId === matchedStorageId,
            )
        ...
        const effectiveParsedData =
          boxResolution?.resolvedParsedData || parsedData;
        ...
        if (this.hasPlaceholderItemTarget(effectiveParsedData)) {
          return {
            ...
            scope,
            clarification:
              this.buildMissingItemNameClarification(effectiveParsedData),
        ...
        if (boxResolution?.kind === 'explicit-bulk-family') {
          ...
          for (const sourceItem of targetedItems) {
            const matchingItems = boxResolution.matchedBoxRecords.map(
              (box: any) => {
                const dbItem = existingContext.items.find(
                  (existingItem: any) =>
                    typeof existingItem === 'object' &&
                    (existingItem as any).boxId === (box as any).id &&
                    this.normalizeLookupName((existingItem as any).name) ===
                      this.normalizeLookupName(sourceItem.name),
                );
            ...
            const missingItemBox = matchingItems.find(
              (entry: any) => !entry.dbItem || typeof entry.dbItem === 'string',
            );
            if (missingItemBox) {
              return {
                intent: 'decrement',
            ...
            const quantities = matchingItems.map(
              (entry: any) => (entry.dbItem as any).quantity ?? 0,
            );
            const sameQuantity = quantities.every(
              (quantity: number) => quantity === quantities[0],
            );
            const displayItemName = this.toTitleCase(sourceItem.name);
            const quotedBoxes = bulkBoxNames.map(
              (name: string) => `'${this.toTitleCase(name)}'`,
            );
            if (sameQuantity) {
              const currentQuantity = quantities[0] ?? 0;
              const nextQuantity = Math.max(
                0,
                currentQuantity - (sourceItem.quantity || 1),
              );
        ...
        for (const item of effectiveTargetedItems) {
          const targetBoxName =
            effectiveParsedData.boxes?.length === 1
          ...
          const dbItem = matchedBoxRecord
            ? existingContext.items.find(
                (existingItem: any) =>
                  typeof existingItem === 'object' &&
                  (existingItem as any).boxId ===
                    (matchedBoxRecord as any).id &&
                  this.normalizeLookupName((existingItem as any).name) ===
    ...
    if (
      ...
      const storageId =
        typeof matchedStorageRecord === 'object'
          ? (matchedStorageRecord as any).id
          : null;
      const scopedBoxes = storageId
        ? (existingContext?.boxes || []).filter(
            (box: any) =>
              typeof box === 'object' && (box as any).storageId === storageId,
          )
        : existingContext?.boxes || [];
      const boxResolution = this.resolveNumberedBoxReference(
        parsedData,
    ...
    const boxesWithQty = (parsedData.boxes || []).filter(
      (b: any) => b.quantity && b.quantity > 1,
    );
    if (boxesWithQty.length > 0) {
      ...
      const repeatedItemSummaries = (parsedData.items || [])
        ...
        .filter(Boolean);
      const repeatedItemSuffix =
        repeatedItemSummaries.length > 0
    ...
    const expandedOriginalNames = new Set(
      (expandedBoxes || []).map((eb: any) => eb.originalName.toLowerCase()),
    );
    ...
    if (existingContext) {
      if (intent === 'create') {
        const matchedStorageRecord = parsedData.storageName
          ? existingContext.storages.find(
              (s: any) =>
                this.normalizeLookupName(typeof s === 'string' ? s : s.name) ===
                this.normalizeLookupName(parsedData.storageName),
            )
          : null;
        const matchedStorageId =
        ...
        const scopedBoxes = matchedStorageId
          ? existingContext.boxes.filter(
              (b: any) =>
                typeof b === 'object' &&
                (b as any).storageId === matchedStorageId,
            )
          : [];
        ...
        if (parsedData.storageName) {
          ...
          if (match) {
            suggestions.push(
              `Storage '${parsedData.storageName}' already exists. Did you mean to update it?`,
            );
            if (
        ...
        parsedData.boxes?.forEach((box: any) => {
          ...
          const match = scopedBoxes.find(
            (b: any) =>
              this.normalizeLookupName(typeof b === 'string' ? b : b.name) ===
              this.normalizeLookupName(box.name),
          );
          if (match) {
            suggestions.push(
              `Box '${box.name}' already exists. Did you mean to update it?`,
            );
          } else {
          ...
          const match = matchedBoxRecord
            ? existingContext.items.find(
                (it: any) =>
                  ...
                  this.normalizeLookupName((it as any).name) ===
                    this.normalizeLookupName(itemName),
              )
            : null;
          if (match) {
            const quantityToAdd =
            ...
            const explicitQuantity =
              typeof item === 'string' ? false : Boolean(item.explicitQuantity);
            const currentQuantity =
              typeof match === 'string' ? null : (match.quantity ?? null);
            ...
            if (explicitQuantity && currentQuantity !== null) {
              ...
              suggestions.push(
                `Item '${itemName}' already exists and no quantity change was requested.`,
              );
            }
            suggestions.push(
              `Item '${itemName}' already exists. Did you mean to update it?`,
            );
    ...
    if (
      ...
      if (
        matchedStorageSummaries.length > 0 ||
        matchedItemSummaries.length > 0
      ) {
        confirmation = [
          ...matchedStorageSummaries,
          ...matchedItemSummaries,
          ...pendingCreateSummaries,
        ].join(' ');
      }
    }
  ...
  private hasConversationalEntityNoise(parsedData: any): boolean {
    ...
    for (const text of entityTexts) {
      ...
      suspiciousTokenCount += tokens.filter((token) =>
        suspiciousWords.has(token),
      ).length;
  ...
  private resolveNumberedBoxReference(
    ...
    kind:
      ...
      | 'single-family'
      | 'ambiguous-family'
      | 'explicit-bulk-family'
      | 'missing'
      | 'missing-explicit-family';
    ...
    const exactMatch = scopedBoxes.find(
      (box: any) =>
        this.normalizeLookupName(typeof box === 'string' ? box : box.name) ===
    ...
    if (exactMatch) {
      return {
        ...
        resolvedParsedData: this.applyResolvedBoxNames(parsedData, [
          typeof exactMatch === 'string' ? exactMatch : exactMatch.name,
        ]),
      };
    }
    ...
    const familyMatches = scopedBoxes.filter((box: any) => {
      ...
      return (
        familyBase &&
        this.normalizeLookupName(familyBase) ===
          this.normalizeLookupName(requestedBoxName)
      );
    });
    const matchedBoxNames = familyMatches.map((box: any) =>
      typeof box === 'string' ? box : box.name,
    );
  ...
  private applyResolvedBoxNames(
    ...
    const resolvedBoxes = resolvedBoxNames.map((name, index) => ({
      ...
      quantity: null,
      clientRef:
        resolvedBoxNames.length > 1 || replicateItemsAcrossBoxes
  ...
  private buildNumberedBoxSelectionResponse(
    ...
    const quotedBoxes = matchedBoxNames.map(
      (name: string) => `'${this.toTitleCase(name)}'`,
    );
    const targetBoxName = this.toTitleCase(
      parsedData.boxes?.[0]?.name || 'box',
    );
    const storageLabel = parsedData.storageName
      ? ` in storage '${this.toTitleCase(parsedData.storageName)}'`
      : '';
    ...
    return {
      ...
      clarificationOptions: this.buildNumberedBoxSelectionOptions(
        parsedData,
        matchedBoxNames,
      ),
      suggestions,
  ...
  private buildNumberedBoxSelectionOptions(
    parsedData: any,
    matchedBoxNames: string[],
  ) {
  ...
  private buildBulkBoxActionConfirmation(
    intent: string | null,
    parsedData: any,
  ): string {
    const quotedBoxes = (parsedData.boxes || []).map(
      (box: any) => `'${box.name}'`,
    );
    ...
    if (intent === 'increment' && (parsedData.items?.length || 0) > 0) {
      const itemLabels = this.joinHumanList(
        this.getDistinctItemsForConfirmation(parsedData).map(
          (item: any) =>
  ...
  private buildMissingNumberedBoxFamilyClarification(
    parsedData: any,
    boxName: string,
  ): string {
    const storageLabel = parsedData.storageName
  ...
  private buildMissingItemNameClarification(
    ...
    const quotedBoxes = (parsedData.boxes || []).map(
      (box: any) => `'${this.toTitleCase(box.name)}'`,
    );
    const boxLabel =
      quotedBoxes.length === 1
        ? `box ${quotedBoxes[0]}`
  ...
  private hasPlaceholderItemTarget(parsedData: any): boolean {
    ...
    return (
      items.length > 0 &&
      items.every((item: any) => this.isPlaceholderItemName(item?.name))
    );
  }
  ...
  private getNumberedFamilyBaseName(
  ...
  private isSiblingNumberedFamilyVariant(
    ...
    return (
      this.normalizeLookupName(leftBase) === this.normalizeLookupName(rightBase)
    );
  }
  ...
  private normalizeLookupName(value: string | null | undefined): string {
    return this.toSingular((value || '').trim().toLowerCase()).replace(
      /\s+/g,
      ' ',
    );
  }
