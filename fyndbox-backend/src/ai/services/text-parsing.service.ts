...
export class TextParsingService {
  ...
  private parseDirectionalItemPhrase(
    ...
    const itemKeywordPattern =
      this.buildKeywordAlternationPattern(itemKeywords);
    if (itemKeywordPattern) {
      normalizedPhrase = normalizedPhrase
        .replace(new RegExp(`^(?:${itemKeywordPattern})\\s+`, 'i'), '')
        .trim();
    }
  ...
  private extractStructuredDescription(
    ...
    const descriptionPattern =
      this.buildKeywordAlternationPattern(descriptionKeys);
  ...
  parseExtraction(normalizedText: string): any {
    ...
    Object.entries(dict.INTENTS).forEach(([intent, syns]) => {
      syns.forEach((syn) => {
        ...
        while ((match = regex.exec(normalizedText))) {
          ...
          if (!intentsFound.has(key) || match.index < intentsFound.get(key)!)
            intentsFound.set(key, match.index);
        }
      });
    });
    ...
    if (intentsFound.size === 1) {
      ...
      intentsFound.size === 2 &&
      intentsFound.has('create') &&
      intentsFound.has('increment')
    ) {
    ...
    if (!extractedIntent && orderedIntents.length > 0) {
      ...
      const compatibleIntents = compatibleIntentMap[primaryIntent] || [
        primaryIntent,
      ];
      if (
        orderedIntents.every((intentName) =>
          compatibleIntents.includes(intentName),
        )
      ) {
    ...
    const greedyDelimPattern = allKeywords
      .sort((a, b) => b.length - a.length)
      .join('|');
    const splitRegex = new RegExp(
      `(\\b(?:${greedyDelimPattern})\\b|[.,:()]|\\b\\d+\\b)`,
      'gi',
    );
    ...
    const groupedStorageSectionPattern =
      this.buildSectionKeywordAlternationPattern(storageSyns);
    const groupedBoxSectionPattern =
      this.buildSectionKeywordAlternationPattern(boxSyns);
    const groupedItemSectionPattern =
      this.buildSectionKeywordAlternationPattern(itemSyns);
    const keywordFlags = {
      storageKeywordSeen: new RegExp(
        `\\b(?:${groupedStorageSectionPattern})\\b`,
        'i',
      ).test(normalizedText),
      boxKeywordSeen: new RegExp(
        `\\b(?:${groupedBoxSectionPattern})\\b`,
        'i',
      ).test(normalizedText),
      itemKeywordSeen: new RegExp(
        `\\b(?:${groupedItemSectionPattern})\\b`,
        'i',
      ).test(normalizedText),
    };
    ...
    const structuredDescriptionKeys = descKeys.filter(
      (entry) => entry.toLowerCase() !== 'with',
    );
    ...
    if (multiBoxEachMatch?.groups) {
      ...
      const { name: itemName, description: itemDescription } = rawItemName
        ? this.extractStructuredDescription(
            rawItemName,
            structuredDescriptionKeys,
          )
        : { name: '', description: null };
      const { name: boxName, description: boxDescription } =
        this.extractStructuredDescription(
      ...
      return {
        ...
        storageDescription:
          multiBoxEachMatch.groups.storageDesc?.trim() || null,
        boxes: [
          {
            name: boxName,
    ...
    if (
      ...
      directionalIncrementMatch.groups.boxPhrase
    ) {
      const rawBoxPhrase = directionalIncrementMatch.groups.boxPhrase.trim();
      const explicitFamilyBox =
        this.extractExplicitFamilyBoxSelector(rawBoxPhrase);
      const explicitBoxMatch = rawBoxPhrase.match(
        new RegExp(`^(?:${groupedBoxSectionPattern})\\s+(?<boxName>.+)$`, 'i'),
      );
      const resolvedBoxName =
        explicitFamilyBox?.boxName?.trim() ||
    ...
    if (
      ...
      return {
        ...
        totalWords: normalizedText.split(/\s+/).filter((w) => w.length > 0)
          .length,
    ...
    if (groupedSectionMatch?.groups) {
      ...
      const isGenericBoxKeyword =
        genericBoxSectionKeywords.has(boxKeyword) || !!itemSectionMatch;
      ...
      if (itemSectionMatch) {
        ...
        for (const segment of boxSegmentParts) {
          ...
          const hasExplicitBoxKeyword = this.DICTIONARY.ENTITIES.BOX.some(
            (syn) =>
              boxNameLower === syn.toLowerCase() ||
              boxNameLower.startsWith(syn.toLowerCase() + ' '),
          );

          if (!isGenericBoxKeyword && !hasExplicitBoxKeyword) {
            const targetBox = groupedBoxes[groupedBoxes.length - 1];
            const { name: itemName, description: itemDescription } =
              this.extractStructuredDescription(
                boxNameRaw,
                structuredDescriptionKeys,
              );
        ...
        for (const [index, segment] of itemSegmentParts.entries()) {
          ...
          const targetBox =
            groupedBoxes[Math.min(index, groupedBoxes.length - 1)];
          const { name: itemName, description: itemDescription } =
            this.extractStructuredDescription(
        ...
        for (const segment of segmentParts) {
          ...
          const hasExplicitBoxKeyword = this.DICTIONARY.ENTITIES.BOX.some(
            (syn) =>
              nameLower === syn.toLowerCase() ||
              nameLower.startsWith(syn.toLowerCase() + ' '),
          );
      ...
      if (groupedBoxes.length > 0) {
        ...
        return {
          intent:
            extractedIntent ||
            (groupedSectionMatch.groups.intent
              ? groupedSectionMatch.groups.intent.toLowerCase()
              : 'create'),
          storageName: groupedSectionMatch.groups.storage.trim(),
          storageDescription:
            groupedSectionMatch.groups.storageDesc?.trim() || null,
          boxes: groupedBoxes,
          items: groupedItems,
    ...
    const preIntentInContexts: string[] = [];
    ...
    const nameConnectors = new Set(
      dict.CONNECTORS.filter(
        (connector) => !['in', 'to', 'from', 'under'].includes(connector),
      ),
    );
    ...
    const finalize = (fallbackName?: string) => {
      ...
      if (
        genericReferences.includes(trimmed.toLowerCase()) &&
      ...
      if (isParsingDescription) {
        if (currentMode === 'STORAGE') {
          storageDescription = storageDescription
            ? `${storageDescription} ${trimmed}`
            : trimmed;
        } else if (currentMode === 'BOX' && currentBoxCluster.length > 0) {
          const b = boxes.find(
            (x) =>
              x.clientRef === currentBoxCluster[currentBoxCluster.length - 1],
          );
          if (b)
          ...
          (currentMode === 'ITEM' || currentMode === 'NONE') &&
          items.length > 0
        ) {
        ...
        if (currentMode === 'STORAGE' && !storageName) {
          storageName = trimmed;
          boxes.forEach((b) => {
            if (b.orphaned) b.orphaned = false;
          });
    ...
    const detectContextInLookahead = (
      startIndex: number,
    ): 'STORAGE' | 'BOX' | 'ITEM' | null => {
      for (
        let j = startIndex;
        j < Math.min(startIndex + 5, parts.length);
        j++
      ) {
      ...
      if (descKeys.includes(token)) {
        if (overlappingConnectorDescriptionKeys.has(token)) {
          const shouldDescribe =
            currentMode !== 'NONE' && accumulator.trim().length > 0;
          return {
            targetMode: 'NONE',
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (!isParsingDescription && expectNameTokens && currentMode !== 'NONE') {
        ...
        const isEntitySectionHeader =
          (storageSyns.includes(lower) ||
            boxSyns.includes(lower) ||
            itemSyns.includes(lower)) &&
          nextNonEmptyToken === ':';
        ...
        if (
          ...
          !this.GENERIC_ENTITY_LABELS.has(lower)
        ) {
      ...
      if (
        preIntentInContexts.length > 0 &&
        preIntentInContexts[preIntentInContexts.length - 1] === 'pending'
      ) {
        if (conjunctions.includes(lower) && accumulator.trim() && storageName) {
          const trimmed = accumulator
            .replace(/^[,.:;\s]+|[,.:;\s]+$/g, '')
            .trim();
          accumulator = '';
          if (trimmed) {
      ...
      if (conjunctions.includes(lower) || connectors.includes(lower)) {
        if (isParsingDescription) {
          ...
          for (let j = i + 1; j < parts.length; j++) {
            ...
            if (
              next.match(/^\d+$/) ||
              storageSyns.includes(next) ||
              boxSyns.includes(next) ||
              itemSyns.includes(next)
            ) {
              nextIsEntity = true;
              break;
            }
          ...
          if (
            lower === 'for' &&
            (currentMode === 'BOX' || currentMode === 'STORAGE') &&
            accumulator === ''
          ) {
          ...
          if (
            conjunctions.includes(lower) &&
            (currentMode === 'BOX' || currentMode === 'ITEM')
          ) {
            expectNameTokens = true;
          }
          continue;
      ...
      if (part.match(/^\d+$/)) {
        ...
        for (let j = i + 1; j < parts.length; j++) {
          ...
          if (look.match(/^[.,:()]$/)) break;
          if (
            storageSyns.includes(look) ||
            boxSyns.includes(look) ||
            itemSyns.includes(look)
          ) {
            isCount = true;
            break;
          }
          // "3 to box", "12 of eggs", "5 for shipping" -> number is always a count
          if (countConfirmConnectors.includes(look)) {
            isCount = true;
            break;
          }
          if (!allKeywords.includes(look)) {
            isCount = true;
            break;
          }
          break;
        ...
        if (isCount) {
          if (accumulator.trim()) finalize();
          pendingQuantity = parseInt(part, 10);
          continue;
        }
      }
    ...
    const explicitFamilySelector =
      this.extractExplicitFamilyBoxSelector(normalizedText);
    if (explicitFamilySelector) {
      ...
      items = items
        .filter(
          (item: any) =>
            !this.isExplicitFamilySelectorArtifact(
              item?.name,
    ...
    allWords.forEach((w, i) => {
      if (keywordSet.has(w.toLowerCase()) || /^\d+$/.test(w))
        understoodIndices.add(i);
    });
    ...
    const markEntityWords = (text: string) => {
      ...
      for (const ew of words) {
        const idx = allWords.findIndex(
          (w, i) =>
            !understoodIndices.has(i) && w.toLowerCase() === ew.toLowerCase(),
        );
    ...
    boxes.forEach((b: any) => {
      markEntityWords(b.name);
      if (b.description) markEntityWords(b.description);
    });
    items.forEach((it: any) => {
      markEntityWords(it.name);
      if (it.description) markEntityWords(it.description);
    });
    ...
    return {
      ...
      meta: {
        mappingStrategy:
          boxes.length > 1 && items.length >= boxes.length
            ? 'sequential'
            : 'direct',
  ...
  private applySequentialMapping(
    boxRefs: string[],
    itemIdxs: number[],
    items: any[],
  ) {
