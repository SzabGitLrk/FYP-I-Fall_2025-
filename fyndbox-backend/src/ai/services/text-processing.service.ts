...
export class TextProcessingService {
    ...
    lightNormalization(text: string): { normalizedText: string; llmBackup: string; typoCount: number } {
        ...
        const result = tokens.map((token) => {
            ...
            if (corrected !== token.toLowerCase() && corrected !== token) typoCount++;
            return corrected;
    ...
    generateConfirmationSummary(normalizedData: any): string {
        ...
        if (normalizedData.storageName) {
            lines.push(`  Storage: ${normalizedData.storageName}${normalizedData.storageDescription ? ` (${normalizedData.storageDescription})` : ''}`);
        }

        if (normalizedData.boxes?.length > 0) {
            const boxNames = normalizedData.boxes.map((box: any) => box.name).join(', ');
        ...
        if (normalizedData.items?.length > 0) {
            const itemDescriptions = normalizedData.items.map((item: any) => {
                ...
                const box = normalizedData.boxes?.find((entry: any) => entry.clientRef === boxRef);
    ...
    private convertWordsToNumbers(text: string): string {
        ...
        protect.forEach((entry) => {
            result = result.replace(new RegExp(`\\b${entry.word}\\b`, 'gi'), entry.token);
        });
